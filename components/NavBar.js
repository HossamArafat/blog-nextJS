...
export default function NavBar() {
    ...
    const getLinkHeaderStyle = (currPath) => {
        return `rounded-md ${path == currPath && "bg-gray-950/50 text-white"} px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white`
    }
            ...
            <nav className="relative bg-gray-800 after:pointer-events-none after:absolute after:inset-x-0 after:bottom-0 after:h-px after:bg-white/10">
                <div className="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
                    ...
                    <div className="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
                        ...
                        <div className="flex space-x-4">
                            <Link href={'/'} aria-current="page" className={getLinkHeaderStyle('/')}>Home</Link>
                            <Link href='/about' className={getLinkHeaderStyle('/about')}>About</Link>
                            <Link href={'/blog'} className={getLinkHeaderStyle('/blog')}>Blog</Link>
                        </div>
                        </div>
                    </div>
                ...
                <el-disclosure id="mobile-menu" hidden className="block sm:hidden">
                    <div className="space-y-1 px-2 pt-2 pb-3">
                        <Link href={'/'} aria-current="page" className="rounded-md bg-gray-950/50 px-3 py-2 text-sm font-medium text-white">Home</Link>
                        ...
                        <Link href={'/blog'} className="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white">Blog</Link>
                    </div>
                </el-disclosure>
            </nav>
        </div>
